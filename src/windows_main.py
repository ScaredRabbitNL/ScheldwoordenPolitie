...
while os.path.exists(f"logs/api_request/api_request_log_{date_str}+{number}.json"):
    number += 1
os.makedirs(name="logs/api_request/", exist_ok=True)

file_path = f"logs/api_request/api_request_log_{date_str}+{number}.json"

with open(file_path, "w") as response_file:
    response_file.write(formatted)

ai_text_rating = int(response.outputs[0].content)
print(ai_text_rating)
